...
def insert_batch(batch_df, batch_id, session):
    for row in batch_df.collect():
        # Log the row data before insertion
        logging.debug(f"Inserting data: {row}")
        insert_data(session, 
                     user_id=row.user_id, 
    ...
    if spark_conn is not None:
        ...
        if session is not None:
            ...
            streaming_query = (selection_df.writeStream.format("org.apache.spark.sql.cassandra")
                            .foreachBatch(lambda batch_df, batch_id: insert_batch(batch_df, batch_id, session))
